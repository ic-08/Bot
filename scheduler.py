...
def scheduler(): 
    ...
    #testing
    while True:
        if str(datetime.now(pytz.timezone('US/Eastern')).strftime("%H:%M")) == str(datetime.now(pytz.timezone('US/Eastern')).strftime("%H:%M")):
            print("hello time is now 11:00")
            break
        else:
            print(str(datetime.now(pytz.timezone('US/Eastern')).strftime("%H:%M")))
            time.sleep(10)
    ...
    #channel = client.get_channel(842823949037076520) 
    #await message.channel.send('hello') 
    channel.send(file=discord.File(f'assets/backgrounds/temp.png'))
    import os
